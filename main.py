...
def main():
    ...
    model = model_generator.from_pretrained(args.model_name, config=config)
    ...
    # sample_batch_size = int((12 if IS_SQUAD else 32) * (0.5 if IS_LARGE else 1))#调整批次大小
    sample_batch_size = 2 # 设定为 2，以模拟论文中的极小样本
    ...
    # 在完整的开发集/验证集上运行推理
    # 注意：这里使用的是调整后的 head_mask 和 neuron_mask
    ...
    # 保存最终的 Mask
    # 只需要保存 Mask，模型权重其实没变（除了被 Mask 缩放的部分）
